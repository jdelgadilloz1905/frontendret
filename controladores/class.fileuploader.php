...
class FileUploader {
	 ...
	 * @return {Array}
     */
    public function upload($id_incidencia) {

    	return $this->uploadFiles($id_incidencia);
    }
     ...
     *
	 * @public
	 * @param $customKey {String} The file attribute which is also defined in list Input element
	 * @return {Array}
     */
     ...
     * @return {null, Array}
     */
	private function uploadFiles($id_incidencia) {
		...
		if ($this->field['input']) {
			...
			if ($validate === true) {
				...
				for($i = 0; $i < count($this->field['input']['name']); $i++) {
					...
					$metas['name'] = $_SESSION["id"]."_".$id_incidencia."_".$this->generateFileName($this->options['title'], array(
    ...
    private function codeToMessage($code, $file = null) {
	    ...
	    switch ($code) {
	        ...
	        case UPLOAD_ERR_PARTIAL:
	            $message = "El archivo cargado solo fue subido parcialmente";
	            break;
            ...
            case 'accepted_file_types':
                $message = "El tipo de archivo no está permitido para " . $file['old_name'];
                break;
            case 'file_uploads':
                $message = "Opción de carga de archivos en deshabilitado en php.ini";
                break;
            case 'max_file_size':
                $message = $file['old_name'] . " Es demasiado largo";
                break;
            case 'max_files_size':
                $message = "Los archivos son demasiado grandes";
                break;
            case 'max_number_of_files':
                $message = "Se ha superado el número máximo de archivos";
                break;
            case 'required_and_no_file':
                $message = "No se seleccionó ningún archivo. Por favor, seleccione uno";
                break;
			case 'invalid_folder_path':
				$message = "La carpeta de carga no existe o no se puede escribir";
				break;
	        default:
	...
	private function generateFilename($conf, $file, $skip_replace_check = false) {
		...
		$type = $conf[0];
		$length = isset($conf[1]) ? (int) $conf[1] : 20;
     ...
     * @return {String}
     */
    public static function mime_content_type($file, $fromName = false) {
        if (function_exists('mime_content_type') && !$fromName) {
